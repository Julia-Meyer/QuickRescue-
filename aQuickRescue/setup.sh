...
if [ ! -f "packages/frontend/.env.local" ]; then
    cp packages/frontend/.env.example packages/frontend/.env.local 2>/dev/null || echo "No frontend .env.example found"
fi
...
echo "   npm run dev --workspace=packages/frontend"
echo ""
